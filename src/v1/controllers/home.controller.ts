...
const createOne = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const homeContentEntityObj = await generateHomeContentEntityObject(
      createRequest
    );
    const createdContent = await homeContentDao.save(homeContentEntityObj);

    const contentDTO = new HomeContentDTO(
      createdContent,
      createdContent.assets
    );
    ...
    error.origin = error.origin
      ? error.origin
      : "createOne-home-content-base-error";
    error.message = error.message
      ? error.message
      : "createOne-home-content-error";
...
const getHome = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const queryBuilder = HomeContentRepository.createQueryBuilder(
      "home"
    ).innerJoinAndSelect("home.assets", "assets");
    ...
    const homeContentList = homeContentData.map(
      (home) => new HomeContentDTO(home, home.assets)
    );
    return Success(res, {
    ...
    error.origin = error.origin
      ? error.origin
      : "getAll-home-content-base-error";
...
const getOne = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const homeContentDTO = new HomeContentDTO(
      fetchedHomeContent,
      fetchedHomeContent.assets
    );
    return Success(res, {
...
export const updateOne = async (
  ...
  try {
    ...
    if (req.files && (req.files as any)["images"]) {
      ...
      newHomeAssets.push(
        ...homeContent.assets.filter((asset) => asset.type === "image")
      );
    }
    ...
    if (updateRequest.video_urls) {
      ...
      newHomeAssets.push(
        ...homeContent.assets.filter((asset) => asset.type === "video")
      );
    }
    ...
    const updatedHomeContent = await homeContentDao.save(
      updatedHomeContentObject
    );

    const contentDTO = new HomeContentDTO(
      updatedHomeContent!,
      updatedHomeContent!.assets
    );
    return Success(res, {
    ...
    error.origin = error.origin
      ? error.origin
      : "update-home-content-base-error";
...
const deleteOne = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    error.origin = error.origin
      ? error.origin
      : "deleteOne-home-content-base-error";
    error.message = error.message
      ? error.message
      : "deleteOne-home-content-error";
    error.code = error.code ? error.code : 3012;
    next(error);
  }
};
